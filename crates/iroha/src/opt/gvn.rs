          ...
          for inst in insts {
            ...
            match op_data {
              OpData::GEP { base, indices } => {
                // Ignore WeakType GEP since they are generated by LSR and may have type mismatch with other GEPs.
                let is_weak_type = self
                  .cx
                  .get_op(inst)
                  .attrs
                  .iter()
                  .any(|attr| matches!(attr, Attr::WeakType));
                if !is_weak_type
                  && indices.len() == 1
