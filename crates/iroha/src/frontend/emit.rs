  ...
  fn emit(&mut self, node_id: NodeId) -> Option<Operand> {
        ...
        Node::UnaryOp { op, operand, .. } => {
          ...
          match op {
            ast::Op::Int2Float => match lit {
              ...
              _ => panic!("Int2Float operand must be Int literal: {:?}", ast[*operand]),
            },
            ...
            _ => unreachable!(
              "Only Int2Float and Float2Int should be used in global initializer casts"
            ),
          }
        }
        _ => panic!(
    ...
    match &self.ast[node_id] {
      ...
      Node::FnDecl {
        ...
        self
          .builder
          .set_current_func(Some(Operand::Func(self.program.funcs.add(Function::new(
            name.clone(),
            false,
            typ.clone(),
          )))));
...

        {
          ...
          for node_id in params.iter() {
            ...
            let param = self.alloc_param((arg_name.clone(), arg_typ.clone()));
            self.builder.create(
              &mut self.program,
              self.builder.current_function,
              mid::Op::new(
          ...
          if let Some(init_values) = &init_values {
            ...
            for range in ranges {
              ...
              self.builder.create(
                &mut self.program,
                self.builder.current_function,
                mid::Op::new(Type::Void, vec![], OpData::Store { addr, value: op_id }),
              );
            }
            ...
            for (chunk_start, chunk_size, init_value) in chunks {
              ...
              self.builder.create(
                ...
                mid::Op::new(
                  ...
                  vec![],
                  OpData::Jump {
                    target_bb: loop_entry,
                  },
                ),
              );

              // loop entry block
              self.builder.set_current_block(loop_entry);
              ...
              // loop body block
              self.builder.set_current_block(loop_body);
                ...
                mid::Op::new(
                  ...
                  OpData::Store {
                    ...
                    value: match init_value.clone() {
                      ...
                      Literal::String(_) => {
                        unreachable!("String literal is not supported in array initialization")
                      }
                    },
                  },
                ),
              ...
              // loop end block
              self.builder.set_current_block(loop_end);
            }
          }
        }
        None
      ...
      Node::If {
        ...
        if let Some(else_id) = else_block {
          ...
          let then_fallthrough = self.builder.current_block;

          self.builder.set_current_block(else_bb);
          self.emit(else_id);
          let else_fallthrough = self.builder.current_block;
      ...
      Node::Break => {
        ...
        self.insert_terminator_and_unplug(OpData::Jump {
          target_bb: loop_info.end_block.unwrap(),
        });
        None
      }
      Node::Continue => {
        ...
        self.insert_terminator_and_unplug(OpData::Jump {
          target_bb: loop_info.while_entry.unwrap(),
        });
        None
      }
      Node::Assign { lhs, rhs } => {
            ...
            Type::Array { .. } => {
              ...
              builder.create(
                ...
                mid::Op::new(
                  ...
                  OpData::GEP {
                    base: ptr,
                    indices: std::iter::once(Operand::Int(0)).chain(indices).collect(),
                  },
                ),
              )
            }
            ...
            ast::Op::Eq => {
              if typ == Type::Bool {
                if lhs_typ == Type::Int && rhs_typ == Type::Int {
                  ...
                  panic!(
                    "Types of lhs and rhs must match for Eq operator: lhs is {:?}, rhs is {:?}",
                    lhs_typ, rhs_typ
                  );
                }
              } else {
            ...
            ast::Op::Ne => {
              if typ == Type::Bool {
                if lhs_typ == Type::Int && rhs_typ == Type::Int {
                  ...
                  panic!(
                    "Types of lhs and rhs must match for Ne operator: lhs is {:?}, rhs is {:?}",
                    lhs_typ, rhs_typ
                  );
                }
              } else {
            ...
            ast::Op::Gt => {
              if typ == Type::Bool {
                if lhs_typ == Type::Int && rhs_typ == Type::Int {
                  ...
                  panic!(
                    "Types of lhs and rhs must match for Gt operator: lhs is {:?}, rhs is {:?}",
                    lhs_typ, rhs_typ
                  );
                }
              } else {
            ...
            ast::Op::Lt => {
              if typ == Type::Bool {
                if lhs_typ == Type::Int && rhs_typ == Type::Int {
                  ...
                  panic!(
                    "Types of lhs and rhs must match for Lt operator: lhs is {:?}, rhs is {:?}",
                    lhs_typ, rhs_typ
                  );
                }
              } else {
            ...
            ast::Op::Ge => {
              if typ == Type::Bool {
                if lhs_typ == Type::Int && rhs_typ == Type::Int {
                  ...
                  panic!(
                    "Types of lhs and rhs must match for Ge operator: lhs is {:?}, rhs is {:?}",
                    lhs_typ, rhs_typ
                  );
                }
              } else {
            ...
            ast::Op::Le => {
              if typ == Type::Bool {
                if lhs_typ == Type::Int && rhs_typ == Type::Int {
                  ...
                  panic!(
                    "Types of lhs and rhs must match for Le operator: lhs is {:?}, rhs is {:?}",
                    lhs_typ, rhs_typ
                  );
                }
              } else {
